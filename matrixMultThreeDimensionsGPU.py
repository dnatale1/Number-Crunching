...
#
# The last entry in the PYMATRIXRESULT3D file is: 50,704,469,048,159,710,000,000,000.
#
...
# 1. Generate Tensor T
T = torch.zeros((2437, 2437, 2437), dtype=torch.int64)
elements_A = 2433 * 2434 * 2435

# Create a flattened sequence [10, 20, 30, ...] and reshape it to fit the slice
seq_A = torch.arange(10, elements_A * 10 + 1, 10, dtype=torch.int64).view(2433, 2434, 2435)
T[:2433, :2434, :2435] = seq_A


sum = 0

# 2. Generate Tensor U
U = torch.zeros((2437, 2437, 2437), dtype=torch.int64)
elements_B = 2435 * 2436 * 2437

# Create the sequence for B and reshape it to fit the slice
seq_B = torch.arange(10, elements_B * 10 + 1, 10, dtype=torch.int64).view(2435, 2436, 2437)
U[:2435, :2436, :2437] = seq_B
